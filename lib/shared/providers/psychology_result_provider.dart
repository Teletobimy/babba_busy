...
/// 사용자의 모든 검사 결과 스트림
final psychologyResultsProvider = StreamProvider<List<PsychologyTestResult>>((ref) {
  ...
  return firestore
      ...
      .snapshots()
      .map((snapshot) =>
          snapshot.docs.map((doc) => PsychologyTestResult.fromFirestore(doc)).toList());
});

/// 특정 검사 유형의 결과 목록
final resultsByTypeProvider = Provider.family<List<PsychologyTestResult>, String>((ref, testType) {
  final results = ref.watch(psychologyResultsProvider).value ?? [];
  return results.where((r) => r.testType == testType).toList();
});

/// 특정 검사 유형의 최근 결과
final latestResultByTypeProvider = Provider.family<PsychologyTestResult?, String>((ref, testType) {
  final results = ref.watch(resultsByTypeProvider(testType));
  return results.isNotEmpty ? results.first : null;
});

/// 검사 이력 요약 (각 유형별 최근 결과)
final psychologyHistorySummaryProvider = Provider<Map<String, PsychologyTestResult?>>((ref) {
...
/// 심리검사 서비스
final psychologyResultServiceProvider = Provider<PsychologyResultService>((ref) {
...
class PsychologyResultService {
  ...
  CollectionReference? get _resultsRef {
    ...
    return _firestore!.collection('users').doc(_userId).collection('psychology_results');
  }
  ...
  /// 결과 공유 설정 업데이트
  Future<void> updateSharing(String resultId, {
