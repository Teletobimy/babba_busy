...
class CoupleCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final partnerTodos = allTodos.where((t) => t.isAssignedTo(partner.id)).toList();
    ...
    final progressRatio = partnerTotal > 0 ? partnerCompleted / partnerTotal : 0.0;

    // 함께 완료한 할일 (둘 다 participants에 포함)
    final sharedCompleted = allTodos.where((t) =>
      t.isCompleted &&
    ...
    final isFilteredToPartner = ref.watch(selectedMemberFilterProvider) == partner.id;
        ...
        if (current == partner.id) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            ...
            action: SnackBarAction(
              label: '해제',
              onPressed: () => ref.read(selectedMemberFilterProvider.notifier).state = null,
            ),
          ));
        }
      },
              ...
              if (sharedCompleted > 0)
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Icon(Iconsax.heart5, size: 12, color: Colors.pink),
                      const SizedBox(width: 4),
                      Text(
                        '함께 $sharedCompleted개',
                        style: const TextStyle(fontSize: 12, color: Colors.pink, fontWeight: FontWeight.w600),
                      ),
                    ],
                  ),
                ),
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor: AlwaysStoppedAnimation<Color>(
                progressRatio >= 1.0 ? AppColors.successLight : AppColors.accentLight,
              ),
              minHeight: 6,
            ),
          ),
