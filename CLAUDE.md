...
## ⚠️ CRITICAL: Git Workflow Rules (필수 준수)

### 🚫 절대 금지
- **`git push` 명령어는 사용자가 명시적으로 요청하기 전까지 절대 실행하지 않음**
- 커밋 후 자동으로 푸시하지 않음
- "커밋하고 푸시해줘"라고 명확히 요청받지 않으면 푸시 금지

### ✅ 올바른 워크플로우
1. 코드 변경 후 `git commit`만 실행
2. 커밋 완료 후 사용자에게 "커밋 완료. 푸시할까요?" 확인
3. 사용자가 "푸시해" 또는 "push 해줘" 등 명시적으로 요청한 경우에만 `git push` 실행

### 예시
```
❌ 잘못된 예: "수정했습니다. 커밋하고 푸시했습니다."
✅ 올바른 예: "수정했습니다. 커밋 완료했습니다. (커밋 해시: abc123) 푸시할까요?"
```
